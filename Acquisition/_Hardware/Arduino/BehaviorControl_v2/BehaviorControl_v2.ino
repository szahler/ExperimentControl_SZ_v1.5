...
void setup() {
  ...
  pinMode (DAQ_ENCODER_STROBE_PIN, OUTPUT);
  pinMode(DAQ_SOLENOID_L_PIN, OUTPUT);
  pinMode(DAQ_SOLENOID_R_PIN, OUTPUT);
  pinMode(DAQ_OPTO_PIN, OUTPUT);

  pinMode(INTAN_SOLENOID_L_PIN, OUTPUT);
  pinMode(INTAN_SOLENOID_R_PIN, OUTPUT);  

  attachInterrupt(digitalPinToInterrupt(ENCODER_A_PIN), changeA, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ENCODER_B_PIN), changeB, CHANGE);

  Timer1.initialize(10000);   // Initialize Timer1 (Set to 50% of desired frame length in us. E.g. 10000 for 20ms)
        ...
        if (PUFF_FINISHED == false && leftpuff_DURATION > 0){
          if (LEFT_SOLENOID_ON == false) { // if element is off, check whether conditions are right to start
            if (currentTime - startTime >= puff_OFFSET) {
              ...
              digitalWrite(INTAN_SOLENOID_L_PIN, HIGH);              
            ...
            if (currentTime - startTime >= puff_OFFSET + leftpuff_DURATION) {
              ...
              digitalWrite(INTAN_SOLENOID_L_PIN, LOW);              
...
void trigger_callback()
{
  if (SESSION_RUNNING == 1) {
    ...
    digitalWrite(DAQ_ENCODER_STROBE_PIN, (digitalRead(DAQ_ENCODER_STROBE_PIN) ^ 1));
    if (digitalRead(DAQ_ENCODER_STROBE_PIN) == HIGH) {
      byte buf[2];
      buf[0] = counter & 255;
      buf[1] = (counter >> 8)  & 255;
      Serial.write(buf,2);
    ...
    if (digitalRead(CAMERA_TRIGGER_PIN) == HIGH || digitalRead(DAQ_ENCODER_STROBE_PIN) == HIGH) {
      digitalWrite(CAMERA_TRIGGER_PIN, LOW);
      digitalWrite(DAQ_ENCODER_STROBE_PIN, LOW);
    }
  }
}
